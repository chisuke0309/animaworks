...
AnimaWorksでは、ひとつのモデルが全てを担わない。Claude HaikuやHaikuのような強力なモデルはエンジニアやマネージャーとして複雑な推論を担い、GPT-4oやGeminiは情報収集や汎用タスクをこなし、vLLMやOllamaなどのローカルモデルはログ監視や定型業務をコツコツ処理する。
...
- **想起**: メッセージを受け取ると、送信者プロファイル・直近活動・関連知識・スキル・タスクキューなど複数チャネルで関連記憶が自動想起される（プライミング）
...
- **自律**: 各Animaはハートビートとcronで自ら動き、自分の理念で判断する。重い実行は計画と分離し、専用のタスク実行パスで処理する
