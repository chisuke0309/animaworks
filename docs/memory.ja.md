...
### 自動ナレッジファイルローテーション（システムレベル）

`ForgettingEngine` ベースの忘却に加え、システムスケジューラ（`core/supervisor/_mgr_scheduler.py`）が2つの軽量クリーンアップジョブを実行する。

#### エピソードローテーション（毎日 03:30 JST）

7日超えの `episodes/` ファイルを削除する（`recovered_*` ファイルも無条件で削除）。古い日次サマリーが `search_memory` 経由で検索されてエージェントのコンテキストに再注入されるのを防ぐ。

#### ナレッジローテーション（毎日 03:35 JST）

**ファイル名に日付パターン**（ISO形式 `YYYY-MM-DD` または連結形式 `YYYYMMDD`）を含む `knowledge/` ファイルのうち、7日超えのものを削除する。対象はスナップショット型の記録（日次トレンドレポート、統合スナップショット、一時メモ等）で、短期間で鮮度が失われるもの。

```
knowledge/
  trend_20260409.md           ← 7日後に削除（ファイル名に連結日付）
  consolidation_2026-04-09.md ← 7日後に削除（ファイル名にISO日付）
  writing-style-principles.md ← 削除されない（日付パターンなし）
```

`archive/` サブディレクトリも含めて再帰的にスキャンする。日付パターンのないファイルは一切削除しない。

#### エンゲージメントログローテーション（毎月1日 04:00 JST）

`knowledge/engagement_log.md` の `## YYYY-MM-DD` セクションのうち30日超えのものを削除する。ファイルのヘッダー部分は常に保持する。高頻度のエンゲージメントログ（稼働時は1日約80行増加）の無制限肥大化を防ぐ。

#### 週次ナレッジ棚卸し（チーム実施・月曜日）

自動削除に加え、毎週月曜日にAnimaのcronタスクとして人間参加型のナレッジ統合プロセスが実行される。

- **オーケストレーター**: `cicchi`（X事業部）と `maru`（TikTok事業部）が毎週月曜日にそれぞれの担当メンバーへ棚卸し指示を送る
- **各Anima**: 自分の `knowledge/` ディレクトリの全ファイルを読み、組織目標（`common_knowledge/organization/goals.md`）に照らして有効性を判断し、生き残った知識を**要約ではなくテーマ別の統合ファイルとして書き直す**
- **プロセス**: 複数の関連ファイル → テーマごとに1つの統合ファイル；統合元の古いファイルは削除
- **報告形式**: 「統合前: X件 → 統合後: Y件、削除: Z件」
- **集約**: 両オーケストレーターがチーム全体の棚卸し結果を `call_human` 週次報告に含める

これは人間の学習整理に相当する。古いノートを捨てるだけでなく、整合的な理解として書き直す作業。
...
空フィールドは省略される。`from`/`to` は送信者/受信者名（内部では `from_person`/`to_person`）、`channel` はチャネル名、`tool` はツール名、`via` は通知チャネル（human_notifyイベント用）、`meta` は任意メタデータ（`from_type` 等）。
...
| `streaming_journal.py` | `StreamingJournal` | WALベースのクラッシュ耐性ストリーミングLLM出力永続化。`streaming_journal_{session_type}.jsonl` |
