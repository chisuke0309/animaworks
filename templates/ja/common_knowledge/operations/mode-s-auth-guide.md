...
| Anima | credential | mode_s_auth | 認証モード | 理由 |
|-------|-----------|-------------|-----------|------|
| sakura | `"max"` | 省略 | Max plan | マネージャー。API コスト不要 |
| kotoha | `"anthropic"` | `"api"` | API 直接 | 高速ストリーミングが必要 |
| rin | `"bedrock"` | `"bedrock"` | Bedrock | AWS VPC 内からのみアクセス |
