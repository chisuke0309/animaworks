...
`defaults.json` は全ロケール共通で、`model`, `context_threshold`, `max_turns`,
`max_chains`, `conversation_history_threshold` を定義する。

### 利用可能なロール

| ロール | 概要 | デフォルトモデル |
|--------|------|-----------------|
| manager | 委任・報告・エスカレーション判断 | claude-haiku-4-5-20251001 |
| engineer | コード実装・技術設計・テスト | claude-haiku-4-5-20251001 |
| researcher | 情報収集・分析・レポート | claude-haiku-4-5-20251001 |
| writer | 文書作成・コミュニケーション設計 | claude-haiku-4-5-20251001 |
| ops | 監視・異常検知・インシデント対応 | ollama/glm-4.7 |
| general | 汎用タスク（デフォルト） | claude-haiku-4-5-20251001 |
...
1. **作成時** (`create_from_md`): `_apply_role_defaults()` が `permissions.md` と
   ...
   `defaults.json` を `status.json` にマージする。
2. **ロール変更時** (`animaworks anima set-role`): 同上のテンプレートを再適用。
