...
以下のエラーメッセージは**システム障害ではありません**。意図的な制限です：

| エラーメッセージ | 意味 | 対処 |
|----------------|------|------|
| `Error: このrunで既に {相手} にメッセージを送信済みです` | 1run中に同じ相手に2通目を送ろうとした | Boardを使うか、次のrunまで待つ |
| `Error: このrunで既に #{channel} に投稿済みです` | 1run中に同じチャンネルに2投目 | 別チャンネルを使うか、次のrunまで待つ |
| `Error: #{channel} にはクールダウン {N}秒が必要です` | 直近の投稿から時間が足りない | 次のrunまで待つ（ツールは正常） |

**これらはツールが「壊れた」サインではありません。** 次のheartbeatかrunで再試行してください。エスカレーション（cicchiへの緊急報告など）は不要です。
