...
このタスクは**記憶の整理専用**です。以下は絶対に行わないこと:

- `x_post_execute_pending` / `x_post` など **投稿系ツールの実行禁止**
- pending_posts ディレクトリの操作（削除・移動を含む）
- `call_human` による通知送信
- `send_message` によるチームメンバーへのDM送信
- 新しいタスクの起動・委任

pending_postsのファイルを発見しても、**そのまま放置すること**。投稿実行は 08:00/17:00 の専用cronが行う。
