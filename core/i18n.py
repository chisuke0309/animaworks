...
_STRINGS: dict[str, dict[str, str]] = {
    ...
    "handler.dm_already_sent": {"ja": "Error: このrunで既に {to} にメッセージを送信済みです（システム障害ではありません。1run1相手の制限です）。追加の連絡はBoardを使用してください。", "en": "Error: Message already sent to {to} in this run (NOT a system failure — 1 DM per recipient per run). Use Board for additional communication."},
    ...
    "handler.post_already_posted": {"ja": "Error: このrunで既に #{channel} に投稿済みです（システム障害ではありません。1run1チャンネルの制限です）。同一チャネルへの連投はできません。{alt_hint}", "en": "Error: Already posted to #{channel} in this run (NOT a system failure — 1 post per channel per run). No duplicate posts to the same channel.{alt_hint}"},
    "handler.post_cooldown": {"ja": "Error: #{channel} には {ts} に投稿済みです（{elapsed}秒前）。クールダウン {cooldown}秒が必要です（システム障害ではありません。次のrunまで待つか別チャンネルを使用してください）。", "en": "Error: Already posted to #{channel} at {ts} ({elapsed}s ago). Cooldown of {cooldown}s required (NOT a system failure — wait until next run or use a different channel)."},
